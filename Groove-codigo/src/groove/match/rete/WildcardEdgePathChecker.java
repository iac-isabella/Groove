...
public class WildcardEdgePathChecker extends SingleEdgePathChecker {
     ...
     */
    public WildcardEdgePathChecker(ReteNetwork network, Wildcard expression, boolean isLoop) {
    ...
    protected RetePathMatch makeMatch(HostEdge edge) {
        RetePathMatch m;
        if (!this.getExpression().allVarSet().isEmpty()) {
            m = new RetePathMatch(this, edge);
            m.getValuation()
                .put(this.getExpression().allVarSet().iterator().next(), edge.getType());
        } else {
            m = new RetePathMatch(this, edge);
        }
        return m;
