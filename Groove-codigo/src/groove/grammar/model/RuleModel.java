 ...
 * the License.
 *
 * $Id: RuleModel.java 5613 2014-10-28 23:41:19Z rensink $
 */
...
public class RuleModel extends GraphBasedModel<Rule> implements Comparable<RuleModel> {
    ...
    public Set<String> getRecipes() {
        return getGrammar().getControlModel().getRecipes(getFullName());
    }
    ...
    private class LevelTree {
        ...
        private Index createIndex(Condition.Op operator, boolean positive, AspectNode levelNode,
            ...
            Index result = new Index(operator, positive, levelNode, getFullName());
        ...
        private Level1 getLevel(Map<Index,Level1> level1Map, AspectNode node) {
            ...
            if (result == null) {
                ...
                assert result != null : String.format("Level map %s does not contain entry for index %s",
                    level1Map,
                    index);
    ...
    private class Level1 implements Comparable<Level1> {
        ...
        public Level1(Index index, Level1 parent) {
            ...
            this.parent = parent;
            if (parent != null) {
                assert index.getParent().equals(parent.getIndex()) : String.format("Parent index %s should be parent of %s",
                    parent.index,
                    index);
                parent.addChild(this);
            } else {
                assert index.isTopLevel() : String.format("Level with index %s should have non-null parent",
                    index);
            }
        }
        ...
        private void addNodeToParents(AspectNode modelNode) {
            ...
            while (ascendingLevel.modelNodes.add(modelNode)) {
                ...
                assert ascendingLevel.modelNodes != null : String.format("Nodes on level %s not yet initialised",
        ...
        private boolean isForNextLevel(AspectElement elem) {
            ...
            if (!this.index.getOperator().hasPattern()) {
                ...
                result = isInjective() && elem.getKind().inLHS() && elem.getAttrAspect() == null;
            } else {
        ...
        @Override
        public String toString() {
            return String.format("Rule %s, level %s, stage 1", getFullName(), getIndex());
        }
    ...
    private class Level2 {
        ...
        private void processEdge(AspectEdge modelEdge) throws FormatException {
            ...
            if (edgeKind.inLHS()) {
                ...
                if (edgeKind.inRHS()) {
                    // creator edge
                    if (getType().isNodeType(ruleEdge) && this.lhs.containsNode(ruleEdge.source())) {
        ...
        @Override
        public String toString() {
            return String.format("Rule %s, level %s, stage 2", getFullName(), getIndex());
        }
                ...
                outer: for (RuleEdge merger1 : mergers) {
                    for (RuleEdge merger2 : mergers) {
                        ...
                        if (!injective(source1, source2) && !target1.equals(target2)
                            ...
                            errors.add("Non-injectively matched mergers have ambiguous target types",
                                merger1,
                                merger2);
                        }
                    }
            ...
            if (commonVars.isEmpty()) {
                ...
                outer: for (TypeNode one : allTypes) {
                    for (TypeNode two : allTypes) {
                        ...
                        if (!one.getSubtypes().contains(two) && !one.getSupertypes().contains(two)) {
                            result = false;
                            break outer;
                        }
                    }
    ...
    private class Level4 {
        ...
        private RuleGraph getIntersection(RuleGraph parentGraph, RuleGraph myGraph) {
            RuleGraph result = parentGraph.newGraph(getFullName() + "-" + getIndex() + "-root");
        ...
        @Override
        public String toString() {
            return String.format("Rule %s, level %s, stage 4", getFullName(), getIndex());
        }
