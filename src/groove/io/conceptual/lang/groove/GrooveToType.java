...
public class GrooveToType extends TypeImporter {
    ...
    private void buildTypeModel(TypeGraph grooveTypeGraph) throws ImportException {
        ...
        for (Iterator<? extends TypeNode> it = unvisitedNodes.iterator(); it.hasNext();) {
            ...
            if (this.m_types.hasModelType(node.label().text())
                && this.m_types.getModelType(node.label().text()) != ModelType.TypeNone) {
        ...
        for (TypeNode n : grooveTypeGraph.nodeSet()) {
            ...
            case TypeClass:
                ...
                setNodeType(n, c);
                if (n.isAbstract()
                    && this.m_cfg.getConfig().getTypeModel().getProperties().isUseAbstract()) {
        ...
        for (TypeEdge e : edges) {
            ...
            if (this.m_types.getModelType(getLabel(e.source())) == ModelType.TypeClass) {
                ...
                Name fieldName = Name.getName(e.label().text());

                // Default multiplicity is 0..*
                int lower = 0;
                int upper = -1;
                if (e.getOutMult() != null) {
                    lower = e.getOutMult().one();
                    upper = e.getOutMult().two();
                ...
                if (e.isComposite()
                    && this.m_cfg.getConfig().getTypeModel().getProperties().isUseContainment()) {
    ...
    private Type resolveIntermediateType(TypeNode interNode) {
        ...
        if (orderType == OrderType.INDEX) {
            ...
            if (!(t instanceof Container) && hasEdge(valueNode, indexName)) {
                isOrdered = true;
            }
        } else if (orderType == OrderType.EDGE) {
            String nextName = this.m_cfg.getStrings().getNextEdge();
            ...
            // For the last non-recursive container, the ordering edge may be on the value node instead
            if (!(t instanceof Container) && hasEdge(valueNode, nextName)) {
                isOrdered = true;
            }
        }
